...
const Menu = ({
  ...
  const { hasChildren, pathList, targetItems } = useMemo(() => {
    ...
    const permissionFilter = ({ permissionRequest }) =>
      permissionRequest
        ? computedIsPass({
            permissions,
    ...
    const targetItems = items
      .filter(
        ({ children, path, onClick }) =>
          (Array.isArray(children) && children.length > 0) || path || onClick
      )
          ...
          return {
            ...
            label: (
              <Row gutter={4}>
                <Col>
                  {iconType ? (
                    <Icon type={iconType} colorful={iconColorFul} />
                  ) : null}
                </Col>
                ...
                ? children
                    ...
                    .map(({ label, key, path, onClick }) => {
                      ...
                      return {
                        ...
                        label,
                        title: label,
                        onClick: () => handlerClick({ key, path, onClick }),
                      };
                    })
                : null,
          };
  ...
  return (
    <AntdMenu
      className={classnames(style["layout-menu"], {
        [style["no-collapsed"]]: !allowCollapsed,
        [style["has-not-children"]]: !hasChildren,
      })}
      selectedKeys={selectKeys}
      defaultOpenKeys={
        defaultOpenKeys || (hasChildren ? items.map(({ key }) => key) : [])
      }
      items={targetItems}
      mode="inline"
